...
TypesElement.prototype.addChild = function(child) {
    ...
    if(child.includes){
        var self=this;
        child.includes.forEach(function(include){
            if(!self.schemas[include.namespace]){
                var s=self.schemas[include.namespace]=new SchemaElement('schema');
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns) {
    var self = this,
        ...
        xsiNilAttr = ' xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" ' + ' xsi:nil="true" ';
    ...
    if (Array.isArray(obj)) {
        ...
        for (var name in obj) {
            var child = obj[name];
            var xsiNil = !child ? xsiNilAttr : null;
            parts.push(['<',ns,name,xmlnsAttrib,xsiNil,'>'].join(''));
